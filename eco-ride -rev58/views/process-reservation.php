...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $carType = $_POST['carType'] ?? ""; // Get the car type from the form
    ...
    // Modify the SQL query based on car type selection
    if ($carType === "electric") {
        // Query for electric cars
        $sqlExact = "
        SELECT * FROM drivers
        WHERE date = ?
        AND departure_time = ?
        AND remaining_seats >= ?
        AND rating >= ?
        AND eco_friendly = 1"; // Assuming eco_friendly field indicates electric cars
    } elseif ($carType === "non-electric") {
        // Query for non-electric cars
        $sqlExact = "
        SELECT * FROM drivers
        WHERE date = ?
        AND departure_time = ?
        AND remaining_seats >= ?
        AND rating >= ?
        AND eco_friendly = 0"; // Non-electric cars
    } else {
        // Query without filtering for car type (if no car type selected)
        $sqlExact = "
        SELECT * FROM drivers
        WHERE date = ?
        AND departure_time = ?
    ...
    $stmtExact->bind_param("ssii", $date, $departure_time, $passenger, $rating);
    $stmtExact->execute();
    ...
    if (empty($availableDrivers)) {
        ...
        // Execute the query for alternative search
        $stmtAlternative = $db->prepare($sqlAlternative);
        $stmtAlternative->bind_param("isssi", $passenger, $dateWindowStart, $dateWindowEnd, $rating, $date);
...
<main class="main">
    <div class="container">
        <div class="driver-result__wrapper">
            <h1 class="ecoride-prime__title driver__title">Conducteurs Disponibles</h1>
            <div class="driver__wrapper">
                <?php if (!empty($availableDrivers)): ?>
                    ...
                    <?php endforeach; ?>
                <?php else: ?>
                    <div class="no-drivers" style="margin-bottom: 2rem">
                        <p>Aucun conducteur disponible pour les critères sélectionnés. Veuillez choisir une date à partir du 27 février.</p>
                    </div>
                <?php endif; ?>
            </div>
