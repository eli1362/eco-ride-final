...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        document.querySelectorAll('.btn-start-trip').forEach(function (button) {
            button.addEventListener('click', function () {
                let carpoolId = this.getAttribute('data-carpool-id');
                let statusElement = document.querySelector(`.status[data-carpool-id='${carpoolId}']`);
                let actionElement = document.querySelector(`#action-${carpoolId}`);

                if (statusElement && actionElement) {
                    // Change status to "Arrivé à destination"
                    statusElement.innerHTML = '<p style="color:var(--dark-green); font-weight: 550">Arrivé à destination</p>';

                    // Replace Annuler button with "C'est déjà fait"
                    actionElement.innerHTML = '<p style="color:var(--dark-green); font-weight: 550">C\'est déjà fait</p>';
                }
            });
        });
    });
