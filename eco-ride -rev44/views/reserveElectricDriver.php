...
//  **Find the Most Recent Non-Electric Reservation**
$stmtFind = $db->prepare("
...
if ($row = $resultFind->fetch_assoc()) {
    ...
    //**Cancel Only the Most Recent Non-Electric Reservation**
...
// **Check if the New Electric Driver Exists and is Available**
...
if ($resultCheck->num_rows > 0) {
    ...
    // Set the reservation_date: if driver['date'] is empty or NULL, use the current date
    $reservation_date = !empty($driver['date']) ? $driver['date'] : date('Y-m-d');

    // **Reserve the New Electric Driver with Full Details**
    $stmtReserve = $db->prepare("
        INSERT INTO reservations (driver_id, user_id, driver_name, driver_price, departure_time, date, passenger) 
        VALUES (?, ?, ?, ?, ?, ?, ?)
    ");
