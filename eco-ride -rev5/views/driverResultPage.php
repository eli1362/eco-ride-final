...
$date = $_GET['date'] ?? '';
$passenger = intval($_GET['passenger'] ?? 0);
$carType = $_GET['carType'] ?? '';
$time = $_GET['time'] ?? '';

// Fetch drivers with exact match first
$stmt = $db->prepare("
    SELECT * FROM drivers 
    WHERE date = ? 
    AND remaining_seats >= ? 
...
// If no exact matches, suggest nearby dates
if (empty($drivers)) {
    $stmt = $db->prepare("
        SELECT * FROM drivers 
            ...
            <div>
                ...
                <?php } else { ?>
                    <?php foreach ($drivers as $driver) { ?>
                        <div>
                            <img src="../public/assets/images/image/<?php echo $driver['photo']; ?>" alt="Driver Image">
                            <p>Name: <?php echo $driver['name']; ?></p>
                            ...
                            <form action="reserveDriver.php" method="POST">
                                ...
                                <input type="hidden" name="user_id" value="USER_ID"> <!-- Replace with actual user ID -->
                                <input type="hidden" name="depart" value="<?php echo htmlspecialchars($depart, ENT_QUOTES, 'UTF-8'); ?>">
                                <input type="hidden" name="destination" value="<?php echo htmlspecialchars($destination, ENT_QUOTES, 'UTF-8'); ?>">
                                ...
                                <input type="hidden" name="passenger" value="<?php echo htmlspecialchars($passenger, ENT_QUOTES, 'UTF-8'); ?>">
                                <input type="hidden" name="car_type" value="<?php echo htmlspecialchars($carType, ENT_QUOTES, 'UTF-8'); ?>">
