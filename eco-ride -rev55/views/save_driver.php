...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $user_id = isset($_SESSION['user_id']) && $role === 'both' ? $_SESSION['user_id'] : NULL;
    ...
    $stmt = $db->prepare("INSERT INTO drivers (name, photo, rating, remaining_seats, price, date, departure_time, arrival_time, eco_friendly, plate_number, registration_date, model, color, brand, smoker, animals, custom_preferences,user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

    $stmt->bind_param("ssdidsssisssssiisi", $name, $photo, $rating, $remaining_seats, $price, $date, $departure_time, $arrival_time, $eco_friendly, $plate_number, $registration_date, $model, $color, $brand, $smoker, $animals, $custom_preferences,$user_id);
