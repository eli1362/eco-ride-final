...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (isset($_POST['cancel_carpool'])) {
        ...
        $_SESSION['success_message'] = "Le covoiturage a été annulé avec succès.";
    }
}
?>
                ...
                <table class="history-table__history-page history-table__history-carpool">
                    ...
                    <?php foreach ($rows as $row): ?>
                        <tr>
                            ...
                            <td class="status" id="status-<?= $row['carpool_id'] ?>" data-carpool-id="<?= $row['carpool_id'] ?>">
                                <?php if ($row['status'] === 'pending'): ?>
                                    <button class="btn-start-trip btn-cancel-history btn-cancel" data-carpool-id="<?= $row['carpool_id'] ?>">Démarrer le trajet</button>
                            ...
                            <td id="action-<?= $row['carpool_id'] ?>">
                                <?php if ($row['status'] !== 'finished'): ?>
                                    <form method="POST" action="cancel_carpool.php" class="cancel-form"
                                          onsubmit="return confirm('Êtes-vous sûr de vouloir annuler ce covoiturage?');">
                                        <input type="hidden" name="carpool_id" value="<?= htmlspecialchars($row['carpool_id']) ?>">
                                        <button type="submit" class="btn-cancel btn-cancel btn-cancel-history">Annuler</button>
                                    </form>
                                <?php else: ?>
                                    <span style="color: grey;">C'est déjà fait</span>
                                <?php endif; ?>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        // Handle the "Démarrer le trajet" button click for both desktop and mobile
        document.querySelectorAll('.btn-start-trip').forEach(button => {
            button.addEventListener('click', function () {
                const carpoolId = this.getAttribute('data-carpool-id');

                // For desktop: hide the "Démarrer le trajet" button and show "Arrivé à destination"
                const statusCell = document.querySelector(`#status-${carpoolId}`);
                if (statusCell) {
                    ...
                    statusCell.appendChild(arrivedText);
                }

                // For mobile: hide the "Démarrer le trajet" button and show "Arrivé à destination"
                const mobileStatusCell = document.querySelector(`#mobile-status-${carpoolId}`);
                if (mobileStatusCell) {
                    this.style.display = 'none';  // Hide the button on mobile
                    const arrivedText = document.createElement('p');
                    arrivedText.textContent = 'Arrivé à destination';
                    arrivedText.style.color = 'var(--dark-green)';
                    arrivedText.style.fontWeight = '550';
                ...
                // Change the "Annuler" button to "C'est déjà fait" on both desktop and mobile
                ...
                // Optionally, you can make an AJAX request to update the status in the database
            });
        });
    });
</script>
